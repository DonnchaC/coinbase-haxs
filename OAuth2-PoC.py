...
def retrieveOAuthForm(oauth_url, cookie):
    ...
    if r.status_code == 200:
        ...
        oauth_form['action'] = 'https://coinbase.com' \
            + oauth_form['action']  # Change the relative URL of the form to an absolute URL
        form_autosubmit = \
            '<script type="text/javascript">document.forms[0].submit();</script>'
        return str(oauth_form) + form_autosubmit  # Send the OAuth authorization form to the user and autosubmit it.
    else:
        # Stop if we could not retrieve the OAuth authorize form
        return 'The OAuth form could not be retrieved, maybe the current requesting user has already authorised this form. Error: ' \
            + str(r.status_code)
...
def callbackPage():
    ...
    print 'Callback code: ' + str(code_token)  # Log code to stdout
    access_token = getAccessToken(code_token)
    if len(access_token) == 64:  # Check we got an access token in the expected format
        print 'Access token: ' + str(access_token)  # Log access_token to stdout
        ...
        print results
        return render_template('result.html', results=results)  # + balance_data.text
    else:
